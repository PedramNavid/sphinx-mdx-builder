...
class MdxTranslator(SphinxTranslator):
    def __init__(self, document: nodes.document, builder: "MdxBuilder") -> None:
        ...
        self.github_url = self.config.mdx_github_url
    ...
    def get_source_github_url(self, objname: str, modname: str, fullname: str) -> str | None:
        ...
        """
        if not self.show_source_links or not modname or not self.github_url:
            return None
    ...
    def depart_document(self, node: nodes.document) -> None:
        title = next(iter(node.nameids.keys()), self.builder.config.mdx_default_title)
    ...
    # TODO: Make these admonition styles configurable for different documentation themes
